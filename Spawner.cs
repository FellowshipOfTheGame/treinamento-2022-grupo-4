...
public IEnumerator SpawnEnemies()
    {
        while (true) //tava assim na internet, nao sei se precisa disso
        {
            ...
            if (variableStorage.GetComponent<VariableStorage>().inimigosRemanescentes == 0)     //se nao tiver mais inimigos na tela, inicia um round novo
            {
                ...
                qualSpawn = Random.Range(0, 3); //seleciona um spawner pro round
                variableStorage.GetComponent<VariableStorage>().ChangeSpawn(qualSpawn); //salva o valor na Storage pra determinar qual caminho o inimigo deve seguir

                yield return new WaitForSeconds(1f);  //espera um pouco antes de sawnar o inimigo para nao ter clipping
                ...
                //qual spawn: 0-top 1-east 2-down 3-west
                    ...
                    switch (qualInimigo)    //verificacao de qual inimigo spawnar
                    {
                        case 0: //spawn normal
                            for (int i2 = 0; i2 <= 3 + variableStorage.GetComponent<VariableStorage>().round; i2+=2)   //ver relacao de com quantos comeca (3+round) e aumenta um inimigo por dois rounds (+=2)
                            {
                                ...
                                yield return new WaitForSeconds(0.75f);  //espera um pouco antes de sawnar o inimigo para nao ter clipping
